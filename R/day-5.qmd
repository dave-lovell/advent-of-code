...
get_destinations <-
  function(destination_range_start, destination_range_end, map_tibble){
        ...
        dre <- destination_range_end
         ...
         if(nrow(viable_source_ranges != 0) &&
            is.na(max(viable_source_ranges$min_source))){
           foo <<- viable_source_ranges
         }
         
         if(nrow(viable_source_ranges) == 0 ||
            dre > max(viable_source_ranges$max_source)){
           
           new_min <- max(viable_source_ranges$min_source) + 1
           
           viable_source_ranges <- 
             add_row(viable_source_ranges,
                     source_range_start = new_min,
                     destination_range_start = new_min,
                     min_source = new_min,
                     max_source = dre)
         }
        
         if(nrow(viable_source_ranges) == 0 ||
            drs < min(viable_source_ranges$min_source)){
           
           new_max <- min(viable_source_ranges$min_source) - 1
           
           viable_source_ranges <- 
             add_row(viable_source_ranges,
                     source_range_start = drs,
